...
FrameNest is a local-first, privacy-conscious, cross-platform library for video and animated media. It is currently in a foundation-stage, pre-alpha state. A minimal Poetry package scaffold exists with centralized configuration, a FastAPI application factory, a Uvicorn loopback-first runtime wired through `framenest.server`, a typed health endpoint tested in-process and through a real local listener, and a runnable development server command (`poetry run framenest-server`). There is no functional user application, database, deployment, systemd unit, or Tailscale integration yet.
