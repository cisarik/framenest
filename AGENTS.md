...
FrameNest is a local-first, privacy-conscious, cross-platform library for video and animated media. It is currently in a foundation-stage, pre-alpha state. A minimal Poetry package scaffold exists with centralized configuration, a FastAPI application factory, a Uvicorn loopback-first runtime wired through `framenest.server`, a typed health endpoint tested in-process and through a real local listener, a runnable development server command (`poetry run framenest-server`), a minimal SQLAlchemy Core/Alembic SQLite persistence foundation with explicit database commands (`poetry run framenest-db status` and `poetry run framenest-db migrate`), a development operator catalog command (`poetry run framenest-catalog`) for local device and library registry operations plus read-only library scan preview (`framenest-catalog library scan-preview`), a minimal pure-domain identity package, a pure-domain `Device` entity, a pure-domain `Library` entity with device-local `LibraryRoot` locators, application `DeviceRepository` and `LibraryRepository` ports, an application scan-preview boundary with a standard-library filesystem scanner per [ADR-0014](docs/adr/0014-safe-library-scan-preview.md), synchronous SQLAlchemy Core registry adapters, and Alembic migration revisions through `0003` for the initial `devices` and `libraries` tables. There is no functional user application, persistent media catalog, storage-volume registration, media persistence from scanning, HTTP API, deployment, systemd unit, or Tailscale integration yet.
