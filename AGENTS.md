...
FrameNest is a local-first, privacy-conscious, cross-platform library for video and animated media. It is currently in a foundation-stage, pre-alpha state. A minimal Poetry package scaffold exists with centralized configuration, a FastAPI application factory, and a typed health endpoint tested in-process. There is no functional user application, runnable server process, database, or deployment yet.
