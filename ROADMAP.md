...
Persistence strategy is accepted through [ADR-0010](docs/adr/0010-initial-persistence-foundation.md). The minimal SQLAlchemy/Alembic migration foundation is implemented. The current schema head is revision `0015`: catalog tables and the automatic built-in `Processed` collection are established through `0007`, while `0008` through `0015` add durable upload, validation, byte-identity, duplicate-disposition, publication-provenance, upload-to-catalog linkage, and optional durable automatic media-analysis run foundations.
