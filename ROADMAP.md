...
Persistence strategy is accepted through [ADR-0010](docs/adr/0010-initial-persistence-foundation.md). The minimal SQLAlchemy/Alembic migration foundation is implemented. The current schema head is revision `0017`: catalog tables and the automatic built-in `Processed` collection are established through `0007`, while `0008` through `0017` add durable upload, validation, byte-identity, duplicate-disposition, publication-provenance, upload-to-catalog linkage, optional durable automatic media-analysis run foundations, first-class still-image media kinds, content classification, acquisition source, movie genres, and movie-identification analysis provenance.
